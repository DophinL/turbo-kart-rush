...
**An arcade kart racer in the spirit of Mario Kart, built with Three.js. This fork replaces Zippy Nova's original procedural kart with a game-ready 3D kart generated by [Tripo](https://www.tripo3d.ai/), while retaining the original driver, physics and seven other kart variants.**

When Zippy is selected, the character screen briefly shows the original kart for comparison and points to the upgraded 3D asset. The comparison disappears automatically before racing and can be reopened from the “Made with Tripo” credit.
...
Open **https://kart.jaredliu.me/** in a desktop browser with WebGL2 (Chrome, Edge, Firefox or Safari). Click or press Enter on the title screen, choose one of eight racers, pick a circuit and difficulty, then Start Race. A keyboard or a gamepad works.
...
- **Eight racers** in three weight classes, each with their own kart, colours and handling. Zippy Nova uses the Tripo-generated showcase kart; the other seven retain their original procedural vehicles.
...
│   ├── kart/                  Kart physics, models, Tripo loader, input and roster
...
├── public/models/tripo/       Tripo-generated showcase kart
├── public/showcase/           before image and generation preview
...
- **Procedural foundation**: tracks, effects, audio, textures and seven karts are still generated in code. Zippy's vehicle is loaded from a compressed GLB generated with Tripo.
...
Every push to `main` runs `.github/workflows/deploy.yml`, which type-checks, builds and publishes `dist/` to GitHub Pages. This fork is also configured for Vercel and the `kart.jaredliu.me` custom domain. Vite uses a relative `base`, so the build works from either host.

## Screenshots

| Tripo upgrade | In race |
| --- | --- |
| ![Zippy Nova Tripo kart with the original comparison](docs/screenshots/tripo-zippy-showcase.png) | ![Zippy Nova Tripo kart on the starting grid](docs/screenshots/tripo-zippy-race.png) |
...
The original source remains under its MIT license. Generation details and provenance for the added Tripo asset are recorded in [ASSET_PROVENANCE.md](ASSET_PROVENANCE.md).
